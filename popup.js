...
async function fetchAssignments() {
    ...
    for (const c of courses) {
        try {
            ...
            for (const a of list) {
                if (a.due_at) allAssignments.push({ ...a, course_name: c.name });
            }
...
document.getElementById('fetch').addEventListener('click', async () => {
    ...
    try {
        ...
        prioritized.forEach(a => {
            ...
            li.textContent = `${a.name} — ${a.course_name} — due ${new Date(a.due_at).toLocaleString()}`;
            console.log(a.name, a.due_at, new Date(a.due_at).toString());
            list.appendChild(li);
        });
    } catch (err) {
